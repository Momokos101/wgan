...
class NetModel(tf.keras.Model):
    ...
    def build_generator_wc(self):
        ...
        in_var = tf.keras.layers.Concatenate(axis=-1)([c_var, z_var])
    ...
    def generate_wc(self, c_var):
        ...
        with condition
        '''
        # 根据输入条件的批次大小生成噪声，而不是固定的 batch_size
        # 获取条件的批次大小
        if isinstance(c_var, tf.Tensor):
            c_var_tensor = c_var
        else:
            c_var_tensor = tf.constant(c_var)
        
        # 获取批次大小（优先使用静态形状，如果为 None 则使用动态形状）
        if c_var_tensor.shape[0] is not None:
            batch_size = int(c_var_tensor.shape[0])
    ...
    def build_generator_wca(self):
        ...
        cz_var = tf.keras.layers.Concatenate(axis=1)([c_eps, z_var])  # shape=(N, 200)
